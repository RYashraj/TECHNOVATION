...
const questions = [
    ...
    {q:"Which is a synonym for 'Huge'?", a:["Tiny","Gigantic","Small"], c:1},
    {q:"Red Planet?", a:["Mars","Venus","Jupiter"], c:0}
];

function startQuiz() { score=0; currentQ=0; showQ(); }
function showQ() {
    const q = questions[currentQ];
    document.getElementById('quiz-body').innerHTML = `<h3>${q.q}</h3>` + 
        q.a.map((o,i) => `<button class="opt" onclick="check(${i})">${o}</button>`).join('');
}
function check(idx) {
    if(idx === questions[currentQ].c) score += 10;
    document.getElementById('score').innerText = "Score: " + score;
    currentQ++;
    if(currentQ < questions.length) showQ();
    else document.getElementById('quiz-body').innerHTML = "<h3>Quiz Complete! 🏆</h3>";
}